...
EepromI2c_24LCxxx::EepromI2c_24LCxxx(const EepromI2cDeviceInterface::AddressData &addressData, int byteCapacity) :
...
int EepromI2c_24LCxxx::WriteData(char* data, ushort count, ushort memAddress)
{
    qInfo("Start EEPROM write on i2c %s / 0x%02X / mem address: 0x%04X / size %u...",
          qPrintable(m_addressData.devNodeFileName), m_addressData.i2cAddr, memAddress, count);
    uchar outpBuf[66]; // 2 address bytes, max 64 byte data
    struct i2c_msg Msgs = {.addr = m_addressData.i2cAddr, .flags = I2C_M_RD, .len =  5, .buf = outpBuf }; // 1 message
    struct i2c_rdwr_ioctl_data EEPromData = {.msgs = &(Msgs), .nmsgs = 1 };
    int toWrite = count;

    if ( I2CTransfer(m_addressData.devNodeFileName, m_addressData.i2cAddr, &EEPromData) )
    ...
    while (toWrite && (memAddress < getByteSize())) {
        ...
        int r;
        for (int i = 0; i < 100; i++) {
            r = I2CTransfer(m_addressData.devNodeFileName, m_addressData.i2cAddr, &EEPromData, true);
            if(r == I2C_IO_OK)
                break;
...
int EepromI2c_24LCxxx::Reset()
{
    qInfo("Start EEPROM reset on i2c %s / 0x%02X...",
          qPrintable(m_addressData.devNodeFileName), m_addressData.i2cAddr);
...
int EepromI2c_24LCxxx::ReadData(char* data, ushort count, ushort memAddress)
{
    ...
    struct i2c_msg Msgs[2] = { {.addr = m_addressData.i2cAddr, .flags = 0,.len = 2,.buf = &(outpBuf[0])}, // 2 messages (tagged format )
                              {.addr = m_addressData.i2cAddr, .flags = (I2C_M_RD+I2C_M_NOSTART), .len = blockReadLen, .buf = &(inpBuf[0])} };
    ...
    while (toRead) {
        ...
        if ( I2CTransfer(m_addressData.devNodeFileName, m_addressData.i2cAddr, &EEPromData) == I2C_IO_OK ) {
